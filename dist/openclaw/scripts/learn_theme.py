"""learn_theme.py — extract a Writer-compatible theme from a WeChat article URL.
...
def fetch_article(url: str, timeout: int = 20) -> "BeautifulSoup":
    ...
    The article title is attached as ``content._writer_title`` (empty string
    if not found).
    """
...
def main():
    ...
    title = getattr(content, "_writer_title", "")
    if title:
        print(f"Title: {title}")
