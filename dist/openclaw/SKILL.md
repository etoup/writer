...
**路径约定**：本文档中 `{baseDir}` 指本 SKILL.md 所在的目录（即 Writer 的根目录）。
...
- 用户说"学习我的修改" → `读取: {baseDir}/references/learn-edits.md`。用户在 `output/` 的 markdown 文件中修改后执行。
- 用户说"验证排版"/"检查主题质量" → `python3 {baseDir}/scripts/theme_quality.py {theme_name}`，输出主题评分和改进建议
...
- 用户说"检查一下"/"自检"/"这篇文章怎么样" → 对最近一篇生成的文章（或用户指定的文章）执行自检，输出生成报告：
  1. `python3 {baseDir}/scripts/article_diagnose.py {article_path}`
...
- 用户说"更新"/"更新 Writer"/"升级" → 在 `{baseDir}` 执行 `git pull origin main`，完成后告知版本变化
- 用户说"看看数据"/"数据报告"/"数据面板" → `python3 {baseDir}/scripts/data_report.py`，生成 HTML 可视化数据报告（发文趋势、平台分布、质量演变、框架使用、热门话题等）
- 用户说"看看数据 --days 30" → 只看最近 30 天数据
- 用户说"看看数据 --json" → 输出 JSON 格式数据
- 用户说"公众号复盘"/"微信数据" → `python3 {baseDir}/scripts/wechat_review.py`，生成公众号专属数据复盘报告（标题策略分析、框架效果对比、质量趋势、优化建议）
- 用户说"生成小红书封面"/"小红书封面" → `python3 {baseDir}/scripts/xhs_cover_gen.py --title "{标题}" --style {风格}`，生成 3:4 比例封面图（支持 minimalist/bold/gradient/dark/warm 五种风格）
- 用户说"检查版本" → `python3 {baseDir}/scripts/version_check.py`，显示当前版本和更新信息
- 用户说"查看错误"/"错误日志" → `python3 {baseDir}/scripts/error_logger.py --show`，显示最近错误记录
...
**读取平台规范**：
```
读取: {baseDir}/platforms/{platform}.md
```
...
```bash
python3 {baseDir}/toolkit/image_gen.py --prompt "测试图片：一个简洁的科技风格图标，蓝色调" --output /tmp/writer_test_image.png --size square
```
...
```bash
python3 {baseDir}/scripts/version_check.py --skip-check 2>/dev/null
```

比对本地 `{baseDir}/VERSION` 与远程版本：
- 相同 → 静默通过
...
读取: {baseDir}/platforms/{platform}.md（当前目标平台规范）
```
...
```
读取: {baseDir}/personas/{当前 platform_overrides 的 writing_persona 字段}.yaml
...
保存到 `{baseDir}/output/{date}-{slug}/{platform}.md`（作为中间文件，最终导出在 Step 7 完成）
...
- 各平台文章保存到 `{baseDir}/output/{date}-{slug}/{platform}.md`
- 各平台配图保存到 `{baseDir}/output/{date}-{slug}/{platform}/`
...
根据目标平台，输出以下内容到 `{baseDir}/output/{date}-{slug}/` 目录：
...
```bash
# 单平台导出
python3 {baseDir}/toolkit/cli.py export {markdown} --platform {platform} --output {output_dir} --theme {theme}

# 多平台导出
python3 {baseDir}/toolkit/cli.py export {markdown} --platform all --output {output_dir} --theme {theme}
```
...
```
读取: {baseDir}/references/frameworks.md
读取: {baseDir}/platforms/{platform}.md（所有目标平台规范）
读取: {baseDir}/history.yaml（去重检查）
```
...
```bash
python3 {baseDir}/toolkit/cli.py per-platform "文章主题" --platforms wechat,zhihu,xiaohongshu --framework 对比 --output ./output/per-platform-topic
```
